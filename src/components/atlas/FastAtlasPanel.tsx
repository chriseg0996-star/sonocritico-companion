...
import {
  FAST_ATLAS_SEARCH_PLACEHOLDER,
  buildFastAtlasNavList,
  fastAtlasComparisons,
  fastAtlasEntries,
  fastAtlasFilters,
  filterFastAtlasEntries,
  getFastAtlasEntryById,
} from "@/lib/modules/fast-atlas";
...
export function FastAtlasPanel({ onOpenEntry, onNavListChange }: PanelProps) {
  const [query, setQuery] = useState("");
  const [filterId, setFilterId] = useState<AtlasFilterId>("all");

  const filtered = useMemo(
    () => filterFastAtlasEntries(fastAtlasEntries, query, filterId),
    [query, filterId]
  );

  const navList = useMemo(() => buildFastAtlasNavList(filtered, filterId), [filtered, filterId]);
    ...
    <div className="atlas-workspace">
      <div className="atlas-toolbar-sticky">
        <SearchBar
          value={query}
          onChange={setQuery}
          placeholder={FAST_ATLAS_SEARCH_PLACEHOLDER}
          resultCount={filtered.length}
        />
        <FilterChips filters={fastAtlasFilters} active={filterId} onChange={setFilterId} />
      </div>
...
export function FastCompareSection({
  onOpen,
  onCompare,
}: {
  ...
  return (
    <div className="atlas-compare-grid">
      {fastAtlasComparisons.map((pair) => (
        <ComparisonCard
          key={pair.id}
          pair={pair}
          onOpen={onOpen}
          onCompare={onCompare}
          getEntryById={getFastAtlasEntryById}
        />
      ))}
    </div>
  );
