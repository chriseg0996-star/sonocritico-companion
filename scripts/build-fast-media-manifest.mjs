/**
 * Escanea public/media/fast/{category}/ y genera manifest TypeScript.
 */
import fs from "node:fs";
import path from "node:path";

const ROOT = path.join(process.cwd(), "public", "media", "fast");
const OUT = path.join(process.cwd(), "src", "lib", "atlas", "fast-media-manifest.ts");
...
if (fs.existsSync(ROOT)) {
  for (const category of fs.readdirSync(ROOT, { withFileTypes: true })) {
    ...
    const rel = (file) => `/media/fast/${cat}/${file}`;
...
const body = `/** Auto-generated — npm run media:fast */
...
export const fastMediaManifest: Record<string, ManifestMedia> = ${JSON.stringify(manifest, null, 2)};
...
console.log("FAST manifest:", Object.keys(manifest).length, "slots →", OUT);
