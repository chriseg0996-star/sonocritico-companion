/**
 * Escanea public/media/cardiac/{category}/ y genera manifest TypeScript.
 */
import fs from "node:fs";
import path from "node:path";

const ROOT = path.join(process.cwd(), "public", "media", "cardiac");
const OUT = path.join(process.cwd(), "src", "lib", "atlas", "cardiac-media-manifest.ts");
...
if (fs.existsSync(ROOT)) {
  for (const category of fs.readdirSync(ROOT, { withFileTypes: true })) {
    ...
    const rel = (file) => `/media/cardiac/${cat}/${file}`;
...
const body = `/** Auto-generated — npm run media:cardiac */
...
export const cardiacMediaManifest: Record<string, ManifestMedia> = ${JSON.stringify(manifest, null, 2)};
...
console.log("Cardiac manifest:", Object.keys(manifest).length, "slots →", OUT);
