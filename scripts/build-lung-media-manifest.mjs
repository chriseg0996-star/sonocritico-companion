/**
 * Escanea public/media/lung/{category}/ y genera manifest TypeScript.
 * Prioridad still: webp > jpg > png > gif > svg | clip: webm > mp4 > gif > svg
 */
import fs from "node:fs";
import path from "node:path";

const ROOT = path.join(process.cwd(), "public", "media", "lung");
const OUT = path.join(process.cwd(), "src", "lib", "atlas", "lung-media-manifest.ts");
...
if (fs.existsSync(ROOT)) {
  for (const category of fs.readdirSync(ROOT, { withFileTypes: true })) {
    ...
    const rel = (file) => `/media/lung/${cat}/${file}`;
...
const body = `/** Auto-generated — npm run media:manifest */
...
export const lungMediaManifest: Record<string, ManifestMedia> = ${JSON.stringify(manifest, null, 2)};
...
console.log("Manifest:", Object.keys(manifest).length, "slots →", OUT);
